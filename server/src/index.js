...
// Temporary import v2 endpoint (remove after use)
app.get('/api/run-van-affinity-import-v2', async (req, res) => {
  ...
  const log = [];
  let vehiclesMatched = 0, vehiclesMissed = 0, updated = 0, skipped = 0;
  ...
  for (const a of assignments) {
    ...
    if (a.p1_id === null && a.p2_id === null) {
      log.push(`- "${a.vehicle}" (id=${vid}) — no IDs to set, skip`);
      skipped++;
      continue;
    }

    // Build upsert that only touches provided fields
    if (a.p1_id !== null && a.p2_id !== null) {
      await pool.query(`
        INSERT INTO van_affinity (vehicle_id, primary_driver_1_id, primary_driver_2_id, updated_at)
        VALUES ($1, $2, $3, NOW())
        ON CONFLICT (vehicle_id) DO UPDATE SET
          primary_driver_1_id = $2, primary_driver_2_id = $3, updated_at = NOW()
      `, [vid, a.p1_id, a.p2_id]);
    } else if (a.p1_id !== null) {
      await pool.query(`
        INSERT INTO van_affinity (vehicle_id, primary_driver_1_id, updated_at)
        VALUES ($1, $2, NOW())
        ON CONFLICT (vehicle_id) DO UPDATE SET
          primary_driver_1_id = $2, updated_at = NOW()
      `, [vid, a.p1_id]);
    } else {
      await pool.query(`
        INSERT INTO van_affinity (vehicle_id, primary_driver_2_id, updated_at)
        VALUES ($1, $2, NOW())
        ON CONFLICT (vehicle_id) DO UPDATE SET
          primary_driver_2_id = $2, updated_at = NOW()
      `, [vid, a.p2_id]);
    }

    updated++;
    log.push(`✓ "${a.vehicle}" (id=${vid}) — set ${a.p1_id !== null ? 'p1_id=' + a.p1_id + ' ' : ''}${a.p2_id !== null ? 'p2_id=' + a.p2_id : ''}`);
  }

  res.json({ vehiclesMatched, vehiclesMissed, missedVehicles, updated, skipped, log });
});
