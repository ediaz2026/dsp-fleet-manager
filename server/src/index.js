...
// One-time van affinity import endpoint (remove after use)
app.get('/api/run-van-affinity-import', async (req, res) => {
  ...
  async function findVehicle(name) {
    const { rows } = await pool.query(
      `SELECT id, vehicle_name FROM vehicles WHERE UPPER(vehicle_name) = UPPER($1)`, [name]
    );
    if (rows.length) return rows[0];
    const { rows: partial } = await pool.query(
      `SELECT id, vehicle_name FROM vehicles WHERE UPPER(vehicle_name) LIKE '%' || UPPER($1)`, [name]
    );
  ...
  async function findDriver(name) {
    if (!name) return null;
    const { rows } = await pool.query(
      `SELECT id, first_name, last_name FROM staff WHERE UPPER(first_name || ' ' || last_name) = UPPER($1)`, [name]
    );
    if (rows.length) return rows[0];
    const { rows: partial } = await pool.query(
      `SELECT id, first_name, last_name FROM staff
       WHERE UPPER(first_name || ' ' || last_name) LIKE '%' || UPPER($1) || '%'
          OR UPPER($1) LIKE '%' || UPPER(first_name || ' ' || last_name) || '%'`, [name]
    );
    if (partial.length >= 1) return partial[0];
    return null;
  }
  ...
  const log = [];
  let vehiclesMatched = 0, vehiclesMissed = 0, driversMatched = 0, driversMissed = 0;
  const missedVehicles = [], missedDrivers = [];
  let upserted = 0;

  for (const a of assignments) {
    const vehicle = await findVehicle(a.vehicle);
    if (!vehicle) {
      log.push(`✗ Vehicle "${a.vehicle}" — NOT FOUND`);
      vehiclesMissed++;
      missedVehicles.push(a.vehicle);
      continue;
    }
    vehiclesMatched++;
    log.push(`✓ Vehicle "${a.vehicle}" → ${vehicle.vehicle_name} (id=${vehicle.id})`);

    const p1 = await findDriver(a.p1);
    const p2 = await findDriver(a.p2);

    if (a.p1) {
      if (p1) { driversMatched++; log.push(`  ✓ P1 "${a.p1}" → ${p1.first_name} ${p1.last_name} (id=${p1.id})`); }
      else    { driversMissed++; missedDrivers.push(a.p1); log.push(`  ✗ P1 "${a.p1}" — NOT FOUND`); }
    }
    if (a.p2) {
      if (p2) { driversMatched++; log.push(`  ✓ P2 "${a.p2}" → ${p2.first_name} ${p2.last_name} (id=${p2.id})`); }
      else    { driversMissed++; missedDrivers.push(a.p2); log.push(`  ✗ P2 "${a.p2}" — NOT FOUND`); }
    }
  ...
  res.json({
    vehiclesMatched, vehiclesMissed, missedVehicles,
    driversMatched, driversMissed, missedDrivers,
    upserted, log
  });
});
