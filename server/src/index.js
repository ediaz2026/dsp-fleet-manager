...
app.get('/api/diag-schedule-audit-v2', async (req, res) => {
  const pool = require('./db/pool');

  // 1. Weekly counts (Sunday-based)
  const { rows: weeklyCounts } = await pool.query(`
    SELECT (shift_date - EXTRACT(DOW FROM shift_date)::int)::date as week_start,
      COUNT(*) as total_shifts,
      CASE
        WHEN COUNT(*) BETWEEN 330 AND 360 THEN 'Normal'
        WHEN COUNT(*) > 360 THEN 'High (+' || (COUNT(*) - 347) || ')'
        WHEN COUNT(*) < 330 THEN 'Low (-' || (347 - COUNT(*)) || ')'
      END as status
    FROM shifts
  ...
  const { rows: dupes } = await pool.query(`
    SELECT COUNT(*) as duplicate_groups FROM (
      SELECT staff_id, shift_date, COUNT(*) FROM shifts
      WHERE shift_date >= CURRENT_DATE
      GROUP BY staff_id, shift_date HAVING COUNT(*) > 1
    ) sub
  `);

  // 3. Smarter stale check — driver must have NO recurring row enabling that day
  const { rows: staleSamples } = await pool.query(`
    SELECT
      s.first_name || ' ' || s.last_name as driver,
      sh.shift_date,
      TO_CHAR(sh.shift_date, 'Dy') as day_name,
      sh.shift_type,
      sh.id as shift_id
    FROM shifts sh
    ...
    WHERE sh.shift_date >= CURRENT_DATE
      ...
      AND NOT EXISTS (
        SELECT 1 FROM driver_recurring_shifts drs
        WHERE drs.staff_id = sh.staff_id
          AND (
            (EXTRACT(DOW FROM sh.shift_date)::int = 0 AND drs.sun = true) OR
            (EXTRACT(DOW FROM sh.shift_date)::int = 1 AND drs.mon = true) OR
            (EXTRACT(DOW FROM sh.shift_date)::int = 2 AND drs.tue = true) OR
            (EXTRACT(DOW FROM sh.shift_date)::int = 3 AND drs.wed = true) OR
            (EXTRACT(DOW FROM sh.shift_date)::int = 4 AND drs.thu = true) OR
            (EXTRACT(DOW FROM sh.shift_date)::int = 5 AND drs.fri = true) OR
            (EXTRACT(DOW FROM sh.shift_date)::int = 6 AND drs.sat = true)
          )
      )
  ...
  // 4. Stale shifts per driver
  const { rows: staleByDriver } = await pool.query(`
    SELECT
      s.first_name || ' ' || s.last_name as driver,
      s.id as staff_id,
      COUNT(*) as stale_count,
      array_agg(DISTINCT TO_CHAR(sh.shift_date, 'Dy')) as stale_days
    FROM shifts sh
    JOIN staff s ON s.id = sh.staff_id
    WHERE sh.shift_date >= CURRENT_DATE
      AND UPPER(sh.shift_type) IN ('EDV','STEP VAN','EXTRA','HELPER')
  ...
  // Bonus: For Christian and Melysse, show ALL their recurring rows
  const { rows: christianRecur } = await pool.query(
    `SELECT id, shift_type, sun, mon, tue, wed, thu, fri, sat FROM driver_recurring_shifts WHERE staff_id IN (SELECT id FROM staff WHERE first_name ILIKE 'christian' AND last_name ILIKE 'gavidia')`
  );
  const { rows: melysseRecur } = await pool.query(
    `SELECT id, shift_type, sun, mon, tue, wed, thu, fri, sat FROM driver_recurring_shifts WHERE staff_id IN (SELECT id FROM staff WHERE first_name ILIKE 'melysse')`
  );

  res.json({
    weeklyCounts,
    duplicateGroups: parseInt(dupes[0].duplicate_groups),
    staleSamples,
    staleByDriver,
    christianRecur,
    melysseRecur
